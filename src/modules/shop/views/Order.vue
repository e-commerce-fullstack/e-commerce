      ...
      <div v-for="order in orders" :key="order._id"
        ...
        <div class="bg-gray-50/50 px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div class="flex items-center gap-2">
            <span v-if="order.paymentStatus === 'paid'"
              class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border bg-green-50 text-green-700 border-green-200">
              ✓ PAID
            </span>
            <span :class="getStatusBadgeStyles(order.status)"
              class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border uppercase">
              {{ order.status }}
            </span>
          </div>
        </div>
        ...
        <div class="px-6 py-6">
          <ul class="divide-y divide-gray-100">
            <li v-for="item in order.products" :key="item._id" class="py-3 flex justify-between items-center">
        ...
        <div class="bg-gray-50 px-6 py-4 flex flex-col sm:flex-row justify-between items-center border-t border-gray-100 gap-4">
          <div class="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span class="font-medium">
              Payment: 
              <span :class="order.paymentStatus === 'paid' ? 'text-green-600' : 'text-orange-500'">
          ...
          <div class="flex items-center gap-6">
            <router-link v-if="order.status === 'pending'" :to="`/payment/${order._id}`"
              class="inline-flex items-center px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-xl transition-all shadow-md active:scale-95">
              Pay Now
            </router-link>
